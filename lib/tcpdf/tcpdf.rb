...
module TCPDF
  ...
  class Renderer
    ...
    @@k_path_cache = nil#File.join(Rails.root, 'tmp')

    cattr_accessor :k_path_url_cache
    @@k_path_url_cache = nil#File.join(Rails.root, 'tmp')
    ...
    def AddFont(family, style='', file='')
      ...
      font_desc = FontDescriptor.font(file)
    ...
    def getfontpath(file)
      ...
      fpath = File.join(File.dirname(File.dirname(__FILE__)), 'fonts', file)
      if File.exists?(fpath)
        return fpath
      end
      # Could not find it.
